...
def test_upload_documents_uses_multipart_endpoint(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    doc_ids = client.upload_documents(dataset_id="kb_demo", documents=docs, sync_id="sync123")
    ...
    assert call["headers"]["X-Request-Id"] == "sync123"
    ...
    rendered = file_entry[1].getvalue().decode("utf-8")
    assert "source_url:" in rendered
    assert "sync_id: sync123" in rendered
...
def test_run_documents_calls_run_endpoint(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    client = RagflowClient(base_url="http://localhost:9380", api_key="k")
    client.run_documents(["doc_a", "doc_b"], sync_id="sync999")
    ...
    assert call["headers"]["Authorization"] == "Bearer k"
    assert call["headers"]["X-Request-Id"] == "sync999"
