...
Keep top-level `sync_id` equal to `summary.sync_id` / `rbac.sync_id`, and top-level `status` equal to `summary.status` / `rbac.status` with the same `unknown` default as `tbox_pipelines.notify` when inner `status` is missing. Checked-in examples include string `status` and `sync_id` inside `summary` / `rbac` (mirrors real audit payloads).
