...
Keep top-level `sync_id` equal to `summary.sync_id` / `rbac.sync_id`, and top-level `status` equal to `summary.status` / `rbac.status` with the same `unknown` default as `tbox_pipelines.notify` when inner `status` is missing. Checked-in examples use non-empty `sync_id` on the envelope and inside `summary` / `rbac`, include string `status` / `sync_id` there (mirrors real audit payloads), and include every top-level envelope key plus the nested body key (`pytest` checks presence).
