...
for f in "${samples[@]}"; do
  ...
  echo "validate_webhook_examples.sh: sample {\"event\":\"sample_validate\",\"component\":\"validate_webhook_examples.sh\",\"log_version\":$LOG_VERSION,\"run_id\":\"$(json_escape "$run_id")\",\"index\":$idx,\"total\":$sample_count,\"path\":\"$(json_escape "$f")\",\"sample_type\":\"$(json_escape "$sample_type")\",\"status\":\"ok\",\"elapsed_ms\":$sample_elapsed_ms}"
done
