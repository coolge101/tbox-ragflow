...
echo "validate_webhook_examples.sh: start {\"event\":\"start\",\"component\":\"validate_webhook_examples.sh\",\"log_version\":$LOG_VERSION,\"run_id\":\"$(json_escape "$run_id")\",\"started_at_utc\":\"$(json_escape "$started_at_utc")\",\"cwd\":\"$(json_escape "$ROOT")\",\"schema\":\"$(json_escape "$schema")\",\"schema_exists\":true,\"schema_mtime_utc\":\"$(json_escape "$schema_mtime_utc")\",\"schema_size_bytes\":$schema_size_bytes,\"schema_hash_alg\":\"sha256\",\"schema_sha256\":\"$(json_escape "$schema_sha256")\",\"samples_dir\":\"$(json_escape "$samples_dir")\",\"samples_glob\":\"$(json_escape "$samples_glob")\",\"sort_locale\":\"$(json_escape "$sort_locale")\",\"samples_sorted\":true,\"samples_nonempty\":true,\"precheck_passed\":true,\"samples\":$sample_count,\"sample_count_expected\":$sample_count_expected,\"samples_total_bytes\":$samples_total_bytes}"
